...
\section{接口文档}
\subsection{\cs{dbNewGroup}}
\begin{function}{\dbNewGroup, \dbNewGroup*}
  \begin{syntax}
    \cs{dbNewGroup} \oarg{base group} \marg{group} \{ \\
    ~~\meta{attr1} = \meta{type spec1}, \\
    ~~\meta{attr2} = \meta{type spec2}, \\
    ~~\ldots{} \\
    \} \\
    \cs{dbNewGroup}* \marg{group} \{ \\
    ~~\meta{attr1} = \meta{type spec1}, \\
    ~~\meta{attr2} = \meta{type spec2}, \\
    ~~\ldots{} \\
    \} \\
...
\subsection{\cs{dbNewGroupStyle} 和样式选项}

\begin{function}{\dbNewGroupStyle}
  \begin{syntax}
    \cs{dbNewGroupStyle} \oarg{base styles} \marg{style} \marg{group} \marg{opts}
  \end{syntax}

  为 \meta{group} 定义一个新的样式 \meta{style}，该样式可以基于已有的样式
  \meta{base styles}，比如 |\dbNewGroupStyle[base1, base2]{new-style}{ques}{}|。
\end{function}
...
\begin{verbatim}
  \dbNewGroup{sort-example}{
    name = str,
    birth = date,
    level = int,
    weight = fp,
  }
  \dbNewGroupStyle{sort-style}{sort-example}{
...
\begin{option}{
  ...
  该选项用来设置展示组中每个元素的代码。你可以使用 \cs{dbuse} 来展示属性的值。
\end{option}
...
\begin{option}{
  ...
  该选项用来设置展示组中属性 \meta{attr} 对应数据之前需要执行的代码。\cs{dbuse}
  会在展示属性数据前执行此代码。
\end{option}
...
\begin{option}{
  ...
  该选项用来设置展示组中属性 \meta{attr} 对应数据之后需要执行的代码。\cs{dbuse}
  会在展示属性数据后执行此代码。
\end{option}
...
\begin{verbatim}
  ...
  \begin{dbGroupFilters}
    \dbNewConditional{cond1}{date}{review-point|\Today} % 定义复习条件
    \dbCombineConditionals{filter1}{cond1}              % 定义过滤器
  \end{dbGroupFilters}
  \dbNewGroupStyle{review-style}{ques}{filter=filter1}  % 定义展示样式
\end{verbatim}
\end{function}

\subsection{在 \env{dbGroupFilters} 环境中定义过滤器}

\noindent\DescribeEnv{dbGroupFilters}
\begin{Syntax}*
  |\begin{dbGroupFilters}|\marg{group} \\
  ~~\meta{code}
  |\end{dbGroupFilters}| \\
\end{Syntax}

\env{dbGroupFilters}用来定义过滤器，此环境中定义了 \cs{dbNewConditional} 命令
用来定义条件和 \cs{dbCombineConditionals} 命令用来组合条件定义过滤器。过滤器独
...
\begin{function}{\dbNewConditional, \dbNewConditional*}
  ...
  所绑定的属性，在 \meta{cond spec} 中可以用 \cs{dbal} 指代属性的值。
\end{function}

  对于类型为 |int| 和 |fp| 的属性，两个版本的定义是一致的，\meta{relation} 可
  以是单个关系式，比如 |\dbal > 3|，也可以是组合关系式，比如
  |\dbal > 3 && \dbal < 10.2|。支持的操作符有 |<, >, =, ==, !=, >=, <=, !|。
...
\begin{function}{\dbCombineConditionals}
  ...
  器的相关信息，在展示组的时候可以用 \cs{dbFilterInfo} 指代。
\end{function}
...
\subsection{使用 \cs{dbshow} 展示组}

\begin{function}{\dbshow}
  \begin{syntax}
    \cs{dbshow} \marg{style} \marg{group}
  \end{syntax}
...
\begin{verbatim}
  \dbNewGroupStyle{style-cond1}{group-test}{
...
\begin{verbatim}
  \dbNewGroupStyle{style-cond2}{group-test}{
...
\begin{verbatim}
  \dbNewGroup{group-test}{labels=clist}
  \dbNewGroupStyle{style-cond3}{group-test}{
...
\begin{function}{\dbGroup, \dbFilterName, \dbFilterInfo, \dbIndex}
  \cs{dbGroup} 指代组名，\cs{dbFilterName} 指代过滤器名称，\cs{dbFilterInfo}
  指代过滤器额外信息，\cs{dbIndex} 指代当前索引。
\end{function}
...
\subsection{\cs{dbNewGroupStyle} and Style Options}
...
|\dbNewGroupStyle[base1, base2]{new-style}{ques}{}|.
\end{function}
...
\begin{verbatim}
  ...
  \end{dbGroupFilters}
  \dbNewGroupStyle{review-style}{ques}{filter=filter1}  % define style
\end{verbatim}
\end{function}

\subsection{Define Filters inside \env{dbGroupFilters} Environment}
...
Filters are defined inside \env{dbGroupFilters} environment, inside which,
...
\begin{function}{\dbNewConditional, \dbNewConditional*}
  ...
  Define the conditional named \meta{name} that binds to \meta{attr}. \cs{dbal}
...
\meta{relation} can be a single relation formula, such as |\dbal > 3|, or the
combination of several relation formulas, such as |\dbal > 3 && \dbal < 10.2|.
...
\subsection{Use \cs{dbshow} to Display the Group}
...
\begin{function}{\dbGroup, \dbFilterName, \dbFilterInfo, \dbIndex}
  \cs{dbGroup} represents for the group name, \cs{dbFilterName} represents for
...
\begin{verbatim}
  ...
  \begin{dbGroupFilters}{ques}
    ...
    \dbNewConditional{bad}{count}{\dbal > 3}
  ...
  \dbNewGroupStyle[test]{test-bad}{ques}{
    filter=bad,
